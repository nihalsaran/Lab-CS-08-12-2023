...
def main():
    ...
    linear_search_time = (linear_search_end - linear_search_start) * 1000
    print(f"Linear Search Time on Sorted Array: {round(linear_search_time, 4)} milliseconds")
    ...
    binary_search_time = (binary_search_end - binary_search_start) * 1000
    print(f"Binary Search Time on Sorted Array: {round(binary_search_time, 4)} milliseconds")

    # Measure execution time for Fibonacci Search on sorted array
    fibonacci_search_start = time.time()
    fibonacci_search(sorted_data, target_value)
    fibonacci_search_end = time.time()
    fibonacci_search_time = (fibonacci_search_end - fibonacci_search_start) * 1000
    print(f"Fibonacci Search Time on Sorted Array: {round(fibonacci_search_time, 4)} milliseconds")

    # Measure execution time for Interpolation Search on sorted array
    interpolation_search_start = time.time()
    interpolation_search(sorted_data, target_value)
    interpolation_search_end = time.time()
    interpolation_search_time = (interpolation_search_end - interpolation_search_start) * 1000
    print(f"Interpolation Search Time on Sorted Array: {round(interpolation_search_time, 4)} milliseconds")
